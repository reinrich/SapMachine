...
bool Deoptimization::deoptimize_objects(JavaThread* thread, GrowableArray<compiledVFrame*>* chunk, bool& realloc_failures) {
...
void Deoptimization::revoke_for_object_deoptimization(JavaThread* deoptee_thread, frame fr, RegisterMap* map, JavaThread* thread) {
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (!mark.has_bias_pattern()
      ...
      // We reach here regularily if there's just eliminated locking on obj.
...
void EscapeBarrier::set_objs_are_deoptimized(JavaThread* thread, intptr_t* fr_id) {
...
bool EscapeBarrier::deoptimize_objects(intptr_t* fr_id) {
  return !barrier_active() || deoptimize_objects(deoptee_thread(), fr_id);
}
      ...
      while (vf != NULL) {
        if (vf->is_compiled_frame()) {
          ...
          if ((cvf->not_global_escape_in_scope() || cvf->arg_escape()) &&
              !deoptimize_objects(jt, cvf->fr().id())) {
...
bool EscapeBarrier::deoptimizing_objects_for_all_threads() {
  assert(Threads_lock->owned_by_self(), "Threads_lock required");
  return _deoptimizing_objects_for_all_threads;
}

void EscapeBarrier::set_deoptimizing_objects_for_all_threads(bool v) {
  assert(Threads_lock->owned_by_self(), "Threads_lock required");
...
void EscapeBarrier::sync_and_suspend_one() {
  ...
  {
    MutexLocker ml(EscapeBarrier_lock);
    while (_self_deoptimization_in_progress) {
      EscapeBarrier_lock->wait();
    }
...
class VM_ThreadSuspendAllForObjDeopt : public VM_Operation {
  public:
   ...
   virtual void doit() {
     ...
     EscapeBarrier::set_deoptimizing_objects_for_all_threads(true);
   }
};
...
void EscapeBarrier::resume_all() {
  ...
  MutexLocker l2(EscapeBarrier_lock);
  assert(_self_deoptimization_in_progress, "incorrect synchronization");
  _self_deoptimization_in_progress = false;
  for (JavaThreadIteratorWithHandle jtiwh; JavaThread *jt = jtiwh.next(); ) {
...
bool EscapeBarrier::deoptimize_objects(JavaThread* deoptee, intptr_t* fr_id) {
  if (!barrier_active()) return true;
  ...
  if (!objs_are_deoptimized(deoptee, fr_id)) {
    ...
    bool deoptimized_objects = Deoptimization::deoptimize_objects(ct, vfs, realloc_failures);
